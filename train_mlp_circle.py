...
def make_circles_np(
    n_samples=400,
    inner_radius=1.0,
    outer_radius_min=1.6,
    outer_radius_max=2.2,
    noise=0.05,
    random_seed=0
):
    ...
    r1 = inner_radius * np.sqrt(rng.uniform(0, 1, size=n1))
    ...
    # class 1: outer annulus
    theta2 = rng.uniform(0, 2 * np.pi, size=n2)
    r2 = np.sqrt(
        rng.uniform(outer_radius_min ** 2, outer_radius_max ** 2, size=n2)
    )
